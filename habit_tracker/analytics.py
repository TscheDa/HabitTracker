...
def calculate_streak(
    ...
    if periodicity == Periodicity.DAILY:
        ...
        completed_months = {
            (c.completed_at.year, c.completed_at.month) for c in completions
        }
...
def get_streak_details(
    completions: List[HabitCompletion], periodicity: Periodicity
    ...
    for i, date_obj in enumerate(sorted_dates):
        ...
        if periodicity == Periodicity.DAILY:
            ...
            diff = (date_obj.year * 12 + date_obj.month) - (
                prev_date.year * 12 + prev_date.month
            )
...
def longest_ongoing_streak_for_habit(
    habit: Habit, completions: List[HabitCompletion]
) -> int:
    """Wrapper to calculate streak for a specific habit object."""
    return calculate_streak(completions, habit.periodicity)


def longest_ongoing_streak_overall(
    habits: List[Habit], all_completions: List[HabitCompletion]
) -> int:
    """
